import { NodeViewProps, NodeViewWrapper, BubbleMenu as BaseBubbleMenu, posToDOMRect, findParentNode } from "@tiptap/react";
...
const adjustCellReferencesInFormula = (formula: string, rowOffset: number, colOffset: number): string => {
  ...
  return formula.replace(cellRefRegex, (match, colAbs, colLetter, rowAbs, rowNumber) => {
...
export default function HandsontableView(props: NodeViewProps) {
  ...
  const [data, setData] = useState(() => {
    ...
    if (node.attrs.data && Array.isArray(node.attrs.data) && node.attrs.data.length > 0) {
        ...
        <HotTable
          ...
          beforeChange={(changes, source) => {
            ...
            if (changes && hotTableRef.current) {
              ...
              const isPaste = source && source.toString() === 'CopyPaste.paste' && changes.length > 1;
              let pasteStartRow = 0;
              let pasteStartCol = 0;
              
              if (isPaste && changes[0] && Array.isArray(changes[0]) && changes[0].length >= 2) {
                // Get the starting position of the paste operation
                pasteStartRow = typeof changes[0][0] === 'number' ? changes[0][0] : 0;
                pasteStartCol = typeof changes[0][1] === 'number' ? changes[0][1] : 0;
              }
              
              for (let i = 0; i < changes.length; i++) {
                if (!Array.isArray(changes[i]) || changes[i].length < 4) continue;
                
                const row = typeof changes[i][0] === 'number' ? changes[i][0] : 0;
                const col = typeof changes[i][1] === 'number' ? changes[i][1] : 0;
                ...
                if (typeof newValue === 'string' && newValue.startsWith('=')) {
                  ...
                  if (isPaste) {
                    ...
                    if (rowOffset > 0 || colOffset > 0) {
                      ...
                      formulaText = adjustCellReferencesInFormula(formulaText, rowOffset, colOffset);
                    }
                  }
