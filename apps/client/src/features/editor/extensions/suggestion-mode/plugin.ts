...
export const suggestionModePluginKey = new PluginKey<SuggestionPluginState>(
  "suggestionModeHover",
);
  ...
  props: {
    handleDOMEvents: {
      mousemove(view: EditorView, event: MouseEvent) {
        ...
        const isMouseOverMenu = targetElement?.closest(
        ...
        if (target) {
          ...
          if (
            !pluginState.visible ||
            pluginState.markFrom !== from ||
            pluginState.markTo !== to
          ) {
            view.dispatch(
              view.state.tr.setMeta(suggestionModePluginKey, {
                newState: {
      ...
      mouseleave(view: EditorView, event: MouseEvent) {
        ...
        const isLeavingToMenu = relatedTargetElement?.closest(
          '[data-suggestion-menu="true"]',
        );
        if (isLeavingToMenu) {
          return false;
