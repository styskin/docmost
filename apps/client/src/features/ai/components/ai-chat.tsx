...
export const AIChat = forwardRef<AIChatRef, AIChatProps>(
  ({ showInput = true, onLoadingChange }, ref) => {
    ...
    const [isRestartingRecognition, setIsRestartingRecognition] =
      useState(false);
    ...
    const safelyRestartRecognition = () => {
      ...
      try {
        ...
        console.error(
          "Error stopping speech recognition before restart:",
          error,
        );
        setIsListening(false);
        setIsRestartingRecognition(false);
    ...
    const convertMessagesToApiMessages = (messages: Message[]) => {
      const apiMessages = [];
      let lastAssistantMessageIndex = null;
      for (const m of messages) {
        if (m.role === "user") {
          apiMessages.push({
            role: m.role,
            content: m.segments.map((s) => (s as TextSegment).content).join(""),
          });
          continue;
        }
        // Assistant messages
        lastAssistantMessageIndex = apiMessages.length;
        apiMessages.push({
          role: "assistant",
          content: "",
          tool_calls: [],
        ...
        for (const segment of m.segments) {
          ...
          if (segment.type === "text") {
            if (lastMessage.role === "assistant") {
              lastMessage.content += segment.content;
            } else {
              lastAssistantMessageIndex = apiMessages.length;
              apiMessages.push({
                role: "assistant",
          ...
          // Tool calls
          apiMessages[lastAssistantMessageIndex].tool_calls.push({
            id: segment.id,
            type: "function",
            function: {
              name: segment.name,
    ...
    const handleExternalSubmit = async (userInput: string) => {
      ...
      const apiMessages = convertMessagesToApiMessages(
